  ...
  install(app) {
    ...
    const refresh = (isPortrait: boolean = window.innerWidth < 1024) => {
      ...
      scroll.enable({
        ...
        newScrollElements: document.querySelector<HTMLElement>('[data-asscroll-element]')!,
      });
    };
    ...
    const setVelocity = () => {
      app.globals.scrollVelocity = getVelocity(scroll.currentPos);
    };

    // Use ASScroll as proxy.
    ScrollTrigger.scrollerProxy(scroll.containerElement, {
      scrollTop(value = 0) {
        return arguments.length && scroll ? (scroll.currentPos = value) : scroll.currentPos;
      },
      scrollLeft(value = 0) {
        return arguments.length && scroll ? (scroll.currentPos = value) : scroll.currentPos;
      },
