...
export class Team extends HTMLElement {
  ...
  private translates = Array.from(this.names, (name) => {
  ...
  private scales = Array.from(this.images, (image, index) => {
    ...
    return ScrollTrigger.create({
      ...
      onUpdate: async ({ progress, isActive }) => {
        ...
        await Promise.all(
          this.translates.map((translate, index) =>
            index === this.current ? Promise.resolve() : translate.reverse().then(),
          ),
        );
